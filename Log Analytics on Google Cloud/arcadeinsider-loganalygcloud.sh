...
echo "${BLUE_TEXT}${BOLD_TEXT}   CAMPUSPERKS | INITIATING EXECUTION   ${RESET_FORMAT}"
echo "${BLUE_TEXT}${BOLD_TEXT}=======================================${RESET_FORMAT}"
echo
...
echo "Frontend External IP: ${BLUE_TEXT}${BOLD_TEXT}$EXTERNAL_IP${RESET_FORMAT}"

# Step 5: Test the deployment
echo
echo "${YELLOW_TEXT}${BOLD_TEXT}Step 5: Testing deployment${RESET_FORMAT}"
HTTP_STATUS=$(curl -o /dev/null -s -w "%{http_code}\n" "http://${EXTERNAL_IP}")
echo "HTTP Status Code: ${BLUE_TEXT}${BOLD_TEXT}$HTTP_STATUS${RESET_FORMAT}"
...
echo "2. View logs in the console: ${BLUE_TEXT}https://console.cloud.google.com/logs/storage/bucket?project=${PROJECT_ID}${RESET_FORMAT}"
echo
...
echo
echo "${CYAN_TEXT}${BOLD_TEXT}${UNDERLINE_TEXT}Subscribe: https://www.youtube.com/@CampusPerkss${RESET_FORMAT}"
echo "${WHITE_TEXT}${BOLD_TEXT}CampusPerks – Google Cloud Arcade Labs made easy 🚀${RESET_FORMAT}"
